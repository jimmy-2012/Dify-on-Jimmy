...
import logging
from plugins import *
from plugins import PluginManager # 添加 PluginManager 导入
...
class difytimetask(Plugin):
    ...
    def _load_root_config(self):
        ...
        try:
            ...
            logger.info(f"[difytimetask] Attempting to load root config from: {root_config_path}") # 打印计算出的路径
            if os.path.exists(root_config_path):
                logger.info(f"[difytimetask] Root config file found at: {root_config_path}")
                try: # 添加内层 try-except 捕获读取和解析错误
                    with open(root_config_path, "r", encoding="utf-8") as f:
                        config_data = json.load(f)
                        logger.info(f"[difytimetask] Successfully loaded and parsed root config.")
                        return config_data
                except json.JSONDecodeError as json_err:
                    logger.error(f"[difytimetask] Failed to parse root config file as JSON: {json_err}")
                    return None
                except IOError as io_err:
                    logger.error(f"[difytimetask] Failed to read root config file (IOError): {io_err}")
                    return None
            else:
                logger.error(f"[difytimetask] Root config file does not exist at: {root_config_path}")
                return None
        except Exception as e:
            # 捕获路径计算或其他意外错误
            logger.error(f"[difytimetask] Unexpected error loading root config: {e}", exc_info=True)
            return None
    ...
    def deal_timeTask(self, content, e_context: EventContext):
        ...
        if len(taskId) > 0:
            ...
            taskStr = ""
            # 特殊处理 check_names 任务的描述
            is_check_names_task = taskModel.eventStr.strip().lower() == "check_names"
            if is_check_names_task:
                 taskStr = f"{circleStr} {timeStr} 执行群成员昵称检查" if not taskModel.isCron_time() else f"{circleStr} 执行群成员昵称检查"
            elif taskModel.isCron_time():
            ...
            if is_check_names_task:
                task_type = "插件任务"
                task_target = "CheckNames 插件"
            elif msg.is_group:
                 task_type = "群任务"
                 task_target = group_title
            elif 'user_nickname' in locals():
                 task_type = "个人任务"
                 task_target = user_nickname
            else:
                 task_type = "个人任务"
                 task_target = msg.other_user_nickname or msg.from_user_nickname
    ...
    def runTimeTask(self, model: TimeTaskModel):
        ...
        task_executed = False # 标记任务是否被处理
        is_check_names_task = eventStr.strip().lower() == "check_names"

        # 检查是否是 CheckNames 任务
        if is_check_names_task:
            try:
                # 回退到使用 PluginManager() 获取实例
                plugin_mgr = PluginManager()
                # 添加调试日志，查看此实例中加载了哪些插件
                logger.debug(f"[difytimetask] PluginManager instance in runTimeTask has plugins: {list(plugin_mgr.plugins.keys())}")
                # 使用大写的键名来获取插件，与注册时保持一致
                check_names_plugin = plugin_mgr.plugins.get("CHECKNAMES")
                if check_names_plugin and check_names_plugin.enabled:
                    # --- 添加更多调试信息 ---
                    logger.debug(f"[difytimetask] Found plugin object: {check_names_plugin}")
                    logger.debug(f"[difytimetask] Type of plugin object: {type(check_names_plugin)}")
                    try:
                        method_to_call = getattr(check_names_plugin, 'run_scheduled_task', None)
                        logger.debug(f"[difytimetask] Method 'run_scheduled_task': {method_to_call}")
                        # 检查方法是否已绑定到实例 (im_self in Python 2, __self__ in Python 3)
                        if hasattr(method_to_call, '__self__'):
                             logger.debug(f"[difytimetask] Method is bound to instance: {method_to_call.__self__ is check_names_plugin}")
                        else:
                             logger.debug(f"[difytimetask] Method does not have __self__ attribute.")
                    except Exception as inspect_e:
                        logger.error(f"[difytimetask] Error inspecting plugin method: {inspect_e}")
                    # --- 结束调试信息 ---

                    logger.info(f"[difytimetask] Triggering CheckNames plugin task: {model.taskId}")

                    # --- 修正：检查获取的是类还是实例 ---
                    if isinstance(check_names_plugin, type):
                        # 如果是类，创建实例
                        logger.debug("[difytimetask] Got class type, creating instance...")
                        try:
                            instance = check_names_plugin()
                            result_msg = instance.run_scheduled_task()
                        except Exception as instantiation_e:
                             logger.error(f"[difytimetask] Error instantiating CheckNames plugin: {instantiation_e}", exc_info=True)
                             result_msg = f"Error instantiating CheckNames: {instantiation_e}"
                    else:
                        # 如果已经是实例，直接调用
                        logger.debug("[difytimetask] Got instance, calling method directly...")
                        result_msg = check_names_plugin.run_scheduled_task()
                    # --- 结束修正 ---

                    logger.info(f"[difytimetask] CheckNames plugin task {model.taskId} triggered. Result: {result_msg}")
                    task_executed = True # 标记任务已执行
                    # CheckNames 任务通常不需要回复给用户, 直接返回
                    # 标记任务已被消费
                    ExcelTool().write_columnValue_withTaskId_toExcel(model.taskId, 14, "1")
                    logger.info(f"任务 {model.taskId} (CheckNames) 已成功执行并标记为已消费。")
                    # 释放内存
                    del model
                    gc.collect()
                    return # CheckNames 任务执行后直接返回
                else:
                    logger.warning(f"[difytimetask] CheckNames plugin not found or disabled. Cannot execute task: {model.taskId}")
            except Exception as plugin_e:
                 logger.error(f"[difytimetask] Error executing CheckNames plugin task {model.taskId}: {plugin_e}", exc_info=True)
            # 如果CheckNames插件执行失败或未找到，也应该标记任务完成避免重试，但不标记为已消费
            # 释放内存
            del model
            gc.collect()
            return # CheckNames 任务执行（或尝试执行）后直接返回

        #处理GPT (仅在非 CheckNames 任务时执行)
        ...
        if isGPT:
            ...
            # 标记任务已被消费
            ExcelTool().write_columnValue_withTaskId_toExcel(model.taskId, 14, "1")
            logger.info(f"任务 {model.taskId} (GPT) 已成功执行并标记为已消费。")
            # 释放内存
            del model
            gc.collect()
            return

        #变量 (仅在非 CheckNames 和非 GPT 任务时执行)
        e_context = None
        # 是否开启了所有回复路由
        ...
        #回复处理 (仅在非 CheckNames 和非 GPT 任务时执行)
        reply_text = ""
        replyType = None
        #插件消息 (路由或拓展功能插件产生的回复)
        if e_context:
            ...
            if reply and reply.type:
                ...
                task_executed = True # 如果有插件回复，说明任务被处理了

        #原消息 (如果前面没有插件处理，则执行普通提醒)
        ...
        if task_executed:
            ...
            # 标记任务已被消费
            ExcelTool().write_columnValue_withTaskId_toExcel(model.taskId, 14, "1")
            logger.info(f"任务 {model.taskId} (Normal/Routed) 已成功执行并标记为已消费。")
        else:
             logger.warning(f"任务 {model.taskId} 未被任何处理程序执行。事件内容: {eventStr}")
    ...
    def _get_group_id_by_title(self, group_title):
        ...
        try:
            ...
            # 检查在线状态
            online_response = self.client.check_online(self.app_id)
            logger.info(f"[difytimetask] 检查在线状态: {online_response}")
            
            if online_response.get('ret') != 200 or not online_response.get('data'):
                logger.error("[difytimetask] 当前不在线，请确保已登录")
                return None
    
            # 记录初始参数
            logger.info(f"[difytimetask] 开始查找群: {group_title}")
            
            # 获取所有联系人列表
            contacts_response = self.client.fetch_contacts_list(self.app_id)
            logger.info(f"[difytimetask] 获取联系人列表响应: {contacts_response}")
            
            if contacts_response.get('ret') != 200:
                logger.error(f"[difytimetask] 获取联系人列表失败: {contacts_response.get('msg', '未知错误')}")
                return None
            ...
            # 如果群聊列表为空，尝试将群聊保存到通讯录
            if not chatrooms:
                logger.info("[difytimetask] 群聊列表为空，尝试搜索群聊")
                search_param = {
                    "type": 2,  # 搜索类型：2表示搜索群聊
                    "keyword": group_title  # 搜索关键词
                }
                search_response = self.client._contact_api.search(self.app_id, search_param)
                logger.info(f"[difytimetask] 搜索群聊响应: {search_response}")
                
                if search_response.get('ret') == 200:
                    groups = search_response.get('data', [])
                    for group in groups:
                        if group.get('nickName') == group_title:
                            group_id = group.get('userName')
                            if group_id:
                                # 将群聊保存到通讯录
                                logger.info(f"[difytimetask] 尝试将群聊保存到通讯录: {group_id}")
                                save_response = self.client.save_contract_list(self.app_id, 3, group_id)
                                logger.info(f"[difytimetask] 保存群聊到通讯录响应: {save_response}")
                                if save_response.get('ret') == 200:
                                    return group_id
            
            # 如果群聊已在通讯录中，直接从群聊列表中查找
            for chatroom_id in chatrooms:
                # 获取群聊详细信息
                detail_response = self.client.get_detail_info(self.app_id, [chatroom_id])
                logger.info(f"[difytimetask] 获取群聊详细信息响应: {detail_response}")
            ...
            if search_response.get('ret') != 200:
                logger.error(f"[difytimetask] 搜索群聊失败: {search_response.get('msg', '未知错误')}")
                return None
                
            # 从搜索结果中获取群聊信息
            groups = search_response.get('data', [])
            if not groups:
                logger.error("[difytimetask] 搜索结果为空")
                return None
                
            # 遍历搜索结果，查找完全匹配的群名称
            for group in groups:
                current_name = group.get('nickName', '')
                group_id = group.get('userName', '')
                logger.info(f"[difytimetask] 比对群名称 - 当前群: {current_name}, 目标群: {group_title}")
                
                if current_name == group_title:
                    logger.info(f"[difytimetask] 找到匹配的群: {group_id}")
                    return group_id
            logger.info(f"[difytimetask] 获取联系人列表响应: {contacts_response}")
            ...
            # 从联系人列表中获取所有群聊ID
            all_groups = []
            if 'chatrooms' in contacts_response.get('data', {}):
                all_groups = contacts_response['data']['chatrooms']
            elif 'groups' in contacts_response.get('data', {}):
                all_groups = contacts_response['data']['groups']
                
            logger.info(f"[difytimetask] 找到的群聊列表: {all_groups}")
            
            if not all_groups:
                logger.error("[difytimetask] 未找到任何群聊")
                return None
                
            # 获取群聊详细信息
            detail_response = self.client.get_detail_info(self.app_id, all_groups)
            logger.info(f"[difytimetask] 获取群聊详细信息响应: {detail_response}")
            
            if detail_response.get('ret') != 200:
                logger.error(f"[difytimetask] 获取群聊详细信息失败: {detail_response.get('msg', '未知错误')}")
                return None
                
            # 遍历群聊信息，查找匹配的群名称
            for group_info in detail_response.get('data', []):
                current_name = group_info.get('nickName', '')
                group_id = group_info.get('userName', '')
                logger.info(f"[difytimetask] 比对群名称 - 当前群: {current_name}, 目标群: {group_title}")
                
                if current_name == group_title:
                    logger.info(f"[difytimetask] 找到匹配的群: {group_id}")
                    return group_id
            logger.info(f"[difytimetask] fetch_contacts_list 完整返回数据: {contacts_response}")
            
            if contacts_response.get('ret') != 200:
                logger.error(f"[difytimetask] fetch_contacts_list 返回错误码: {contacts_response.get('ret')}")
                logger.error(f"[difytimetask] 错误信息: {contacts_response.get('msg', '未知错误')}")
                return None
                
            # 从联系人列表中获取群聊列表
            chatrooms = contacts_response.get('data', {}).get('chatrooms', [])
            if not chatrooms:
                chatrooms = contacts_response.get('data', {}).get('groups', [])  # 尝试其他可能的字段名
            ...
            # 使用 get_brief_info 获取群聊信息
            brief_response = self.client.get_brief_info(self.app_id, chatrooms)
            logger.info(f"[difytimetask] get_brief_info 完整返回数据: {brief_response}")
            
            if brief_response.get('ret') != 200:
                logger.error(f"[difytimetask] get_brief_info 返回错误码: {brief_response.get('ret')}")
                return None
                
            # 遍历群聊信息，查找匹配的群名称
            for room_info in brief_response.get('data', []):
                current_name = room_info.get('nickName', '')
                chatroom_id = room_info.get('userName', '')
                logger.info(f"[difytimetask] 正在比对群名称 - 当前群: {current_name}, 目标群: {group_title}")
                
                if current_name == group_title:
                    logger.info(f"[difytimetask] 找到匹配的群, chatroom_id: {chatroom_id}")
                    return chatroom_id
            logger.info(f"[difytimetask] fetch_contacts_list 完整返回数据: {contacts_response}")
