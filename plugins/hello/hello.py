...
class Hello(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        # group_name = msg.from_user_nickname # Incorrect: This gets sender nickname or group ID
        group_name = e_context["context"].get("group_name") # Correct: Get group display name from context kwargs
        ...
        if e_context["context"].type == ContextType.PATPAT:
            # e_context["context"].type = ContextType.TEXT # No longer need to change type
            # e_context["context"].content = self.patpat_prompt # No longer need to change content
            reply = Reply()
            reply.type = ReplyType.TEXT
            reply.content = self.patpat_prompt # Directly use the prompt as reply content
            e_context["reply"] = reply
            e_context.action = EventAction.BREAK_PASS # End event processing and send the reply directly
            # Remove the logic related to BREAK and AI generation
            # if not self.config or not self.config.get("use_character_desc"):
            #     e_context["context"]["generate_breaked_by"] = EventAction.BREAK
            return
